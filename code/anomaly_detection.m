...
while not(isequal(seg,seg_precedent))
    ...
    % 4. Calculer la similarité entre les intensités des pixels du voisinage
    %    et la moyenne d'intensité de la région segmentée
     delta = 12;
        ...
        % 4.4. Trouver les pixels qui satisfont la condition de supériorité
        %      Parmi les pixels candidats du contour
        sup_seg = ind(imgf(ind) < bsup);
        
        % 4.5. Trouver les indices des pixels qui satisfont les deux conditions
        %      à la fois, (càd le critère de similarité)
		candidats = intersect(inf_seg, sup_seg);
        
    % 5. Mettre à jour l'image seg, 
    %    (càd mettre tous les pixels candidats à 1 dans la variable seg)
    seg(candidats) = 1;
